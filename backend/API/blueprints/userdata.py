...
def update_user():
    try:
        ...
        for course_code in new_current_courses:
            if all(course_code != user_course.course.code for user_course in user.current_courses if user_course.course is not None):
        ...
        for course_code in new_previous_courses:
            if all(course_code != user_completed_course.course.code for user_completed_course in user.completed_courses if user_completed_course.course is not None):
                course = Courses.query.filter_by(code=course_code).first()
                if course:
